...
for (let i = 0; i < arr.length; i++) {
  if (arr[i].length > indexPosition) {
    indexPosition = arr[i].length;
    longest = arr.indexOf(arr[i]);
  }
}
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map((item) => {
    newArr.push(
      `${item.charAt(1)}${item.charAt(2)}${item.charAt(3)}${item.charAt(
        6
      )}${item.charAt(7)}${item.charAt(8)}${item.charAt(10)}${item.charAt(
        11
      )}${item.charAt(12)}${item.charAt(13)}`
    );
  });
