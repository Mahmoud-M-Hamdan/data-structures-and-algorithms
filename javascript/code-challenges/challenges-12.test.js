...
const validatePin = (pin) => {
  // Solution code here...
  let regex = /(\b[0-9][0-9][0-9][0-9]\b)/g;
...
const hasNumber = (string) => {
  // Solution code here...
  let regex = /([A-Za-z]{1,100}[0-9]{1,100})/g;
...
const validateEmail = (email) => {
  // Solution code here...
  let regex = /^[_a-z0-9-]+\.?[_a-z0-9-]+?@[a-zA-Z0-9]+.(net|com|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let x = /^((\(\d{3}\))|\d{3})(\s|-)?\d{3}(\s|-)?\d{4}$/;
  let y = x.test(String(phoneNumber));
  return y;
};
